<template>
	<div>
      ...
      "
		 :page-sizes="[10, 20, 30, 40]" :page-size="pagesize" layout="total, sizes, prev, pager, next, jumper" :total="
        tableData
          .filter(
            (data) =>
              !currentSelectOption || data.childrenId == currentSelectOption
          )
          .filter(
            (data) =>
              !search ||
              data.newsTitle.toLowerCase().includes(search.toLowerCase())
          ).length
      "
		 :currentPage="currentPage" @size-change="size_change" @current-change="current_change"></el-pagination>
        ...
        "
			 v-model="currentSelectOption" size="mini" placeholder="请选择子栏目">
				...
				<el-option v-for="item in currentChildNavItems" style="width: 100%" :key="item.classId" :label="item.itemName"
          ...
          z-index: 2;
        "
			 v-model="search" size="mini" placeholder="输入标题关键字搜索" />
			<el-table :data="
          tableData
            .filter(
              (data) =>
...
<script>
	import {getRoleInfoById} from "../api/getRoleInfo.js";
		...
		methods: {
			handleEdit(index, row) {
				this.$router.push({
					...
					params: {
						newsId: row._id,
						roleID: this.currentRoleInfo.modelID,
						NavItems: this.currentChildNavItems,
						type: "edit",
					},
				});
			},
				...
				getRoleInfoById(id)
					.then((result) => {
						if (result.code == 666) {
							...
							this.$options.methods.getChildNavItems(
								this,
								this.$data.currentRoleInfo.modelID
							);
							this.$options.methods.getNewsItems(
								this,
								this.$data.currentRoleInfo.modelID
							);
						}
			...
			getNewsItems(that, parentId) {
				newsItemApi
					.getNewsItemByParentId(parentId)
					.then((result) => {
